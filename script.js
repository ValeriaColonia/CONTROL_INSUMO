...
function guardarIngreso(event) {
    ...
    const registro = {
        ...
        fechaRegistro: registroAnterior
...
function mostrarInventarioActual() {
    ...
    document.getElementById("btnCambiarVistaInv").textContent =
        "Ver Historial de Entradas";

    document.getElementById("tituloInventario").textContent =
        "Inventario Actual";

    document.getElementById("subtituloInventario").textContent =
        "Consulta, edita o elimina los registros existentes.";

    document.getElementById("contenedorTablaInventario").style.display =
        "block";

    document.getElementById("contenedorHistorialEntradas").style.display =
        "none";
...
function eliminarRegistro(id) {
    ...
    /* Eliminar también las salidas asociadas */
...
function registrarSalida(event) {
    ...
    const totalSalidas = salidas
        .filter(
            salida => salida.inventarioId === inventarioId
        )
        .reduce(
            (total, salida) =>
                total + Number(salida.cantidad),
...
function descargarExcelSalidas() {
    ...
    salidas.forEach(salida => {
        ...
        const item = inventario.find(
            inv => inv.id === salida.inventarioId
        );
    ...
    link.setAttribute(
        "download",
        `Reporte_Salidas_${new Date().toISOString().slice(0, 10)}.csv`
    );
